	...
	fun emitBindingUpdate(key: String, data: JsonElement, onComplete: () -> Unit) {
		sendMessage(
			OutgoingMessage.EmitBindingUpdate(
				requestId = listenForAcknowledgement(onComplete), key = key, data = data
			)
		)
	}
			...
			override fun onFailure(webSocket: WebSocket, t: Throwable, response: Response?) {
				when (t) {
					is ConnectException, is SocketTimeoutException, is UnknownHostException -> {
						if (retryCounter > 5 && !isOffline) {
							isOffline = true
							onHasInternet.emit(false)
						}

						println("retrying, counter is at $retryCounter")
						queueRetry()
					}
...
sealed class OutgoingMessage {
	...
	@SerialName("watch")
	data class Watch(
		@SerialName("request_id") val requestId: String, val id: String
	) : OutgoingMessage()
	...
	data class EmitBindingUpdate(
		@SerialName("request_id") val requestId: String, val key: String, val data: JsonElement
	) : OutgoingMessage()
}
